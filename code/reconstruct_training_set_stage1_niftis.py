...
def get_volume_predicted_single_plane(subject, pix2pix_out_dir, model_name, phase='train', epoch='latest'):
    # all image outputs
...
def save_volume_predicted_single_plane(subject, fold, data_dir, output_dir, model_name_stem, plane, phase='train', epoch='latest'):
    ...
    pix2pix_out_dir = os.path.join(data_dir, f'results_stage1_training_set')
...
def iterate_for_each_sub(fold, data_dir, output_dir, model_name_stem, plane, phase='train', epoch='latest'):
    # all image outputs in axial plane
    all_images = os.listdir(os.path.join(data_dir, 'results_stage1_training_set', f'{model_name_stem}_{fold}_axial', f'{phase}_{str(epoch)}', 'images'))
...
def iterate_for_each_plane(fold, data_dir, output_dir, model_name_stem, phase='train', epoch='latest'):
...
def iterate_for_each_fold(data_dir, output_dir, model_name_stem, phase='train', epoch='latest', n_splits=12):
    # get list of folds
    ...
    for fold in folds:
        ...
        output_dir_fold = os.path.join(output_dir, fold, 'reconstructed_stage1_niftis_training_set')
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='Reconstruct nifti volumes from generated stage 1 training set images')
    ...
    if bool(args.parallel) == True:
        ...
        for fold in folds:
            # all image outputs in axial plane
            all_images = os.listdir(os.path.join(args.data, fold, 'results_stage1_training_set', f'{args.model_name_stem}_{fold}_axial', 
            ...
            output_dir_fold = os.path.join(args.output_dir, fold, 'reconstructed_stage1_niftis_training_set')
