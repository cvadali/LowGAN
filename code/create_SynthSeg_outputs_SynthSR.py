...
# generate SynthSeg segmentation of a single SynthSR T1 image
def create_SynthSeg_segmentation_single_image(subject, SynthSeg_dir, image_path, output_dir):
    ...
    output_path = os.path.join(output_dir, f'{subject}_synth_T1_SynthSeg.nii.gz')
...
def iterate_for_all_subs(subs_file, SynthSeg_dir, images_dir, output_dir):
    ...
    for sub in full_subject_list:
        # path to T1 image
        image_path = os.path.join(images_dir, f'{sub}_synth_T1.nii.gz')
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='Create SynthSeg segmentations of SynthSR T1 images')
    ...
    parser.add_argument('-images_dir','--images_dir',
                        help='Directory with SynthSR images',
                        required=True,
                        )
    ...
    if bool(args.parallel) == True:
        ...
        for sub in full_subject_list:
            # path to t1 image
            image_path = os.path.join(os.path.abspath(args.images_dir), f'{sub}_synth_T1.nii.gz')
