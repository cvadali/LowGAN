...
def iterate_for_each_sub(data_dir, output_dir, model_name_stem, plane, phase='test', epoch='latest'):
    ...
    all_images = os.listdir(os.path.join(data_dir, 'results_stage1', f'{model_name_stem}_axial', f'{phase}_{str(epoch)}', 'images'))
...
def iterate_for_each_plane(data_dir, output_dir, model_name_stem, phase='test', epoch='latest'):
    ...
    for plane in planes:
        ...
        iterate_for_each_sub(data_dir, output_dir, model_name_stem, plane, phase, epoch)


# if script is actually run
if __name__ == '__main__':
    # parse command line args
    parser = argparse.ArgumentParser(description='Reconstruct nifti volumes from generated stage 1 images')
    ...
    if bool(args.parallel) == True:
        ...
        # all image outputs in axial plane
        all_images = os.listdir(os.path.join(args.data, 'results_stage1', f'{args.model_name_stem}_axial', 
        ...
        for plane in planes:
            ...
            if os.path.exists(output_dir_plane) == False:
                os.makedirs(output_dir_plane)
            for sub in full_subject_list:
                list_of_arguments.append((sub, os.path.abspath(args.data), output_dir, args.model_name_stem, 
