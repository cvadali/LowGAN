...
def coregister_all_subjects(subjects_list, data_dir, output_dir):
    reshape_dir = os.path.join(data_dir, 'recon_niftis_reshaped')

    if os.path.exists(output_dir) == False:
        os.makedirs(output_dir)

    for sub in subjects_list:
        coregister_all_modalities_single_subject(sub, reshape_dir, output_dir)


# if script is run
if __name__ == '__main__':
    # parse command line args
    parser = argparse.ArgumentParser(description='Coregister reshaped nifti volumes')
    ...
    subjects_list = get_subject_list(os.path.abspath(args.subs_file))
