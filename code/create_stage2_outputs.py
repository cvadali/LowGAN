...
def predict_all_subjects(output_dir, device, images_dir,
                            batch_size, patch_size, patch_overlap, n_splits=12):
    # get list of folds
    folds = [f'fold_{fold}' for fold in range(0, int(n_splits))]

    for fold in folds:
        # all image outputs in axial plane
        all_images = os.listdir(os.path.join(output_dir, fold, 'recon_niftis_reshaped'))
        ...
        weights_path_fold = os.path.join(output_dir, fold, 'stage2_weights', 'UNet_combine_weights_100_ssim-l1.pth')

        output_dir_fold = os.path.join(output_dir, fold, 'recon_niftis_stage2')

        images_dir_fold = os.path.join(output_dir, fold, 'recon_niftis_reshaped_coregistered')
    
        # iterate for each subject
        for sub in full_subject_list:
            predict_single_subject(weights_path_fold, output_dir_fold, device, sub, images_dir_fold,
                                    batch_size, patch_size, patch_overlap)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-output_dir','--output_dir',
                        help='Directory with all outputs',
                        required=True,
                        )
