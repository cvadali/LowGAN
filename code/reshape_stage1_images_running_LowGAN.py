...
def iterate_for_each_plane(subject_list, data_dir, out_dir):
    planes = ['axial', 'coronal', 'sagittal']

    reconstructed_dir = os.path.join(data_dir, 'reconstructed_stage1_niftis')

    if os.path.exists(out_dir) == False:
        os.mkdir(out_dir)

    for plane in planes:
        reshape_multiple_subjects(subject_list, plane, reconstructed_dir, out_dir)
...
if __name__ == '__main__':
    ...
    if bool(args.parallel) == True:
        ...
        reconstructed_dir = os.path.join(os.path.abspath(args.data), 'reconstructed_stage1_niftis')
        ...
        for plane in planes:
            for sub in full_subject_list:
                list_of_arguments.append((sub, plane, reconstructed_dir, os.path.abspath(args.output_dir)))
