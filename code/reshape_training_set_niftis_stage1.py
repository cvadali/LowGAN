...
def iterate_for_each_fold(data_dir, output_dir, n_splits=12):
    # get list of folds
    folds = [f'fold_{fold}' for fold in range(0, int(n_splits))]

    for fold in folds:
        # all image outputs in axial plane
        all_images = os.listdir(os.path.join(data_dir, fold, 'reconstructed_stage1_niftis_training_set', 'recon_axial'))
        ...
        reconstructed_dir_fold = os.path.join(data_dir, fold, 'reconstructed_stage1_niftis_training_set')

        output_dir_fold = os.path.join(output_dir, fold, 'recon_niftis_training_set_reshaped')
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='Reshape nifti volumes from reconstructed stage 1 niftis for training set')
    ...
    if bool(args.parallel) == True:
        ...
        for fold in folds:
            # all image outputs in axial plane
            all_images = os.listdir(os.path.join(args.data, fold, 'reconstructed_stage1_niftis_training_set', 'recon_axial'))
            ...
            reconstructed_dir_fold = os.path.join(args.data, fold, 'reconstructed_stage1_niftis_training_set')

            output_dir_fold = os.path.join(args.output_dir, fold, 'recon_niftis_training_set_reshaped')
